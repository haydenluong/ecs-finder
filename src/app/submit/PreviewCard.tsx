        ...
        >
            <div className="aspect-[3/2] bg-[var(--topic-13)] p-[13px] flex items-start justify-between relative overflow-hidden">
                {image && !decodeFailed ? (
                    ...
                    >
                        ...
                        {decodeFailed ? (
                            <span className="text-[12px] text-text-faint px-3 text-center">{t('preview.previewFailed')}</span>
                        ) : onBrowse && (
                        ...
                        {tooManyFiles && (
                            <span className="text-[11px] text-red-600 px-3 text-center">{t('error.image.tooMany')}</span>
                        )}
                    </div>
                )}
                ...
                <span className={`relative z-[1] text-[10.5px] font-bold uppercase tracking-[0.05em] rounded-[7px] py-1 px-[9px] border ${
                    ...
                    {category ? categoryLabel(category, lang) : t('filters.category')}
                </span>
            ...
            <div className="pt-4 px-[17px] pb-[17px] flex flex-col gap-[11px]">
                <h3 className="font-heading font-bold text-[16px] leading-[1.3] tracking-[-0.005em] text-text m-0 line-clamp-2 min-h-[41.6px]">
                    {name || <span className="text-text-faint font-normal">{t('preview.namePlaceholder')}</span>}
                </h3>
                ...
                <div className="flex gap-3.5 flex-wrap">
                    <span className={`flex items-center gap-1 text-[13px] ${location ? 'text-text-dim' : 'text-text-faint'}`}>
                        ...
                        {location ? locationLabel(location, lang) : t('submit.location')}
                    </span>
                    <span className={`flex items-center gap-1 text-[13px] ${deadline ? 'text-text-dim' : 'text-text-faint'}`}>
                        ...
                        {deadline ? formatDeadlineDisplay(deadline) : t('submit.deadline')}
                    </span>
                </div>
                ...
                <div className="flex gap-1.5 flex-wrap">
                    <span className={`text-[12px] font-semibold py-[3px] px-2.5 rounded-full ${
                        ...
                        {topic ? topicLabel(topic, lang) : t('submit.topic')}
                    </span>
                    {topic && subtopic && (
                        <span className="text-[12px] font-medium py-[3px] px-2.5 rounded-full bg-[var(--topic-07)] text-[var(--topic)] border border-[var(--topic-20)]">{subtopicLabel(subtopic, lang)}</span>
                    )}
                </div>
                ...
                {positions.length > 0 && (
                    <div className="border-t border-border pt-2.5 mt-0.5 text-[12px] text-text-faint">
                        ...
                        <span className="text-accent-2 font-medium text-[12.5px]">
                            {positions.map(pos => positionLabel(pos, lang)).join(' · ')}
                        </span>
                    </div>
                )}
            </div>
